...
const ViewMoreModal = ({ plan, onClose, onReminder, onNotification }) => {
  ...
  const rows = [
    ...
    { label: 'Monthly Amount:', value: plan.monthlyAmount },
    { label: 'Duration Months:', value: plan.durationMonths },
    { label: 'Total Value:', value: plan.totalValue },
    { label: 'Bonus:', value: plan.bonus },
...
const summaryCards = [
  { label: 'Gold', value: '₹311', icon: MdMonetizationOn, gradient: 'linear-gradient(135deg, #f59e0b 0%, #dc2626 100%)' },
  { label: 'Silver', value: '₹311', icon: MdStar, gradient: 'linear-gradient(135deg, #f97316 0%, #e11d48 100%)' },
  { label: 'Diamond', value: '₹311', icon: MdDiamond, gradient: 'linear-gradient(135deg, #ea580c 0%, #be123c 100%)' },
];

const AddEditPlanModal = ({ plan, onClose, onSave, error }) => {
  const isEdit = !!plan;
  const [planName, setPlanName] = useState(plan?.planName ?? '');
  const [monthlyAmount, setMonthlyAmount] = useState(plan?.monthlyAmount ?? '');
  const [durationMonths, setDurationMonths] = useState(plan?.durationMonths ?? '');
  const [totalValue, setTotalValue] = useState(plan?.totalValue ?? '');
  const [bonus, setBonus] = useState(plan?.bonus ?? '');
  const [status, setStatus] = useState(plan?.status ?? 'Active');

  const handleSubmit = (e) => {
    e.preventDefault();
    onSave({ planName, monthlyAmount, durationMonths, totalValue, bonus, status }, plan?.id);
  };
      ...
      <div style={styles.modalBox} className="add-edit-plan-modal-box" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} style={styles.form}>
          ...
          <div style={styles.formGroup}>
            <label style={styles.formLabel}>Plan Name</label>
            <input type="text" value={planName} onChange={(e) => setPlanName(e.target.value)} style={styles.formInput} placeholder="e.g. Silver, Gold" required />
          </div>
          <div style={styles.formGroup}>
            <label style={styles.formLabel}>Monthly Amount</label>
            <input type="text" value={monthlyAmount} onChange={(e) => setMonthlyAmount(e.target.value)} style={styles.formInput} placeholder="e.g. ₹2000" required />
          </div>
          <div style={styles.formGroup}>
            <label style={styles.formLabel}>Duration Months</label>
            <input type="text" value={durationMonths} onChange={(e) => setDurationMonths(e.target.value)} style={styles.formInput} placeholder="e.g. 12 Month" required />
          </div>
          <div style={styles.formGroup}>
            <label style={styles.formLabel}>Total Value</label>
            <input type="text" value={totalValue} onChange={(e) => setTotalValue(e.target.value)} style={styles.formInput} placeholder="e.g. ₹ 10,000" required />
          </div>
          <div style={styles.formGroup}>
            <label style={styles.formLabel}>Bonus</label>
            <input type="text" value={bonus} onChange={(e) => setBonus(e.target.value)} style={styles.formInput} placeholder="e.g. ₹ 5,000" required />
          </div>
          <div style={styles.formGroup}>
          ...
          <div style={styles.modalFooterForm} className="add-edit-plan-modal-footer">
            ...
            <button type="submit" style={styles.modalBtnPrimary}>{isEdit ? 'Update Plan' : 'Add Plan'}</button>
          </div>
        </form>
      </div>
    </div>
  ...
  return (
    <div style={styles.container} className="dashboard-container plans-page">
      ...
      {showPlanModal && (
        <AddEditPlanModal
          plan={editingPlan}
          onClose={() => { setShowPlanModal(false); setEditingPlan(null); setSaveError(null); }}
      ...
      {viewModalPlan && (
        <ViewMoreModal
          ...
          onReminder={() => { /* TODO */ setViewModalPlan(null); }}
          onNotification={() => { /* TODO */ setViewModalPlan(null); }}
        />
      )}
      ...
      <main style={styles.main} className="dashboard-main plans-main">
          ...
          {summaryCards.map((card) => {
            ...
            return (
              <div key={card.label} style={{ ...styles.summaryCard, background: card.gradient }} className="plans-summary-card">
                ...
                <div style={card.label === 'Diamond' ? { ...styles.summaryCardIcon, color: 'rgba(147, 197, 253, 0.95)' } : styles.summaryCardIcon}>
                  <Icon size={32} color={card.label === 'Diamond' ? 'rgba(147, 197, 253, 0.95)' : 'rgba(255,255,255,0.9)'} />
                </div>
              </div>
            );
          })}
        ...
        <div style={styles.tableWrap} className="plans-table-wrap">
          <table style={styles.table} className="plans-table">
            <thead>
              <tr>
                <th style={styles.th}><span className="th-content">Plan Name <MdKeyboardArrowUp size={14} /><MdKeyboardArrowDown size={14} /></span></th>
                <th style={styles.th}><span className="th-content">Monthly Amount <MdKeyboardArrowUp size={14} /><MdKeyboardArrowDown size={14} /></span></th>
                <th style={styles.th}><span className="th-content">Duration Months <MdKeyboardArrowUp size={14} /><MdKeyboardArrowDown size={14} /></span></th>
                <th style={styles.th}><span className="th-content">Total Value <MdKeyboardArrowUp size={14} /><MdKeyboardArrowDown size={14} /></span></th>
                <th style={styles.th}><span className="th-content">Bonus <MdKeyboardArrowUp size={14} /><MdKeyboardArrowDown size={14} /></span></th>
            ...
            <tbody>
              {(loading ? [] : plans).map((row) => (
                <tr key={row.id} style={styles.tr}>
                  <td style={styles.td}>{row.planName}</td>
                  <td style={styles.td}>{row.monthlyAmount}</td>
                  <td style={styles.td}>{row.durationMonths}</td>
                  <td style={styles.td}>{row.totalValue}</td>
                  <td style={styles.td}>{row.bonus}</td>
                  <td style={styles.td}>
        ...
        <ActionMenu
          ...
          onDelete={() => { const row = plans.find((r) => r.id === openActionId); if (row) handleDelete(row); }}
        />
        ...
        <div className="plans-cards" style={styles.cardsWrap}>
          {(loading ? [] : plans).map((row) => (
            <div key={row.id} style={styles.planCard} className="plan-card">
              <div style={styles.planCardRow}><span style={styles.planCardLabel}>Plan Name</span><span>{row.planName}</span></div>
              <div style={styles.planCardRow}><span style={styles.planCardLabel}>Monthly Amount</span><span>{row.monthlyAmount}</span></div>
              <div style={styles.planCardRow}><span style={styles.planCardLabel}>Duration</span><span>{row.durationMonths}</span></div>
              <div style={styles.planCardRow}><span style={styles.planCardLabel}>Total Value</span><span>{row.totalValue}</span></div>
              <div style={styles.planCardRow}><span style={styles.planCardLabel}>Bonus</span><span>{row.bonus}</span></div>
              <div style={styles.planCardRow}><span style={styles.planCardLabel}>Status</span>
        ...
        <ActionMenu
          ...
          onDelete={() => { const row = plans.find((r) => r.id === openCardActionId); if (row) handleDelete(row); }}
        />
